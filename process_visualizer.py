...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=str, 
        required=True,
        help="Pfad zur importierten JSON-Datei"
    )
    parser.add_argument(
        "--client_type",
        type=str,
        ...
        for curved_track in elements.get("curved_track", []):
            curved_track_element = {
                ...
                "geometry": {
                    ...
                    "start_radius": float(str(curved_track.get("Startradius", "inf")).replace("inf", "9999999")),
        ...
        for drainage in elements.get("drainage", []):
            if drainage.get("Typ") == "Pipe":
                ...
                drainage_element = {
                    ...
                    "project": project_name,
                    "properties": {
                        "gefaelle": float(drainage.get("Gefälle", 0.0))
                    }
                }
            else:
                ...
                try:
                    durchmesser = 0.0
                    if isinstance(drainage.get("Durchmesser"), str) and drainage.get("Durchmesser").isdigit():
                    ...
                    drainage_element = {
                        ...
                        "geometry": {
                            ...
                            "height": 2.0,
                            "position": [
                                float(drainage.get("E", 0.0)) if isinstance(drainage.get("E"), (int, float, str)) and str(drainage.get("E", "")).replace(".", "", 1).isdigit() else 0.0,
                                float(drainage.get("N", 0.0)) if isinstance(drainage.get("N"), (int, float, str)) and str(drainage.get("N", "")).replace(".", "", 1).isdigit() else 0.0,
                                float(drainage.get("Z", 0.0)) if isinstance(drainage.get("Z"), (int, float, str)) and str(drainage.get("Z", "")).replace(".", "", 1).isdigit() else 0.0
                            ]
                        },
                        "material": {
                            "name": drainage.get("Material", "Beton"),
...
def convert_client_b_data(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    visualization_data = {
        "scene": {
            "name": "ClientB Infrastrukturszene",
            "elements": []
        }
    }
    ...
    for foundation in elements.get("foundation", []):
        foundation_element = {
            ...
            "properties": {
                "typ": foundation.get("fundament_typ", "")
            }
        }
        visualization_data["scene"]["elements"].append(foundation_element)
    ...
    for drainage in elements.get("drainage", []):
        if drainage.get("typ") == "Pipe":
            ...
            drainage_element = {
                ...
                "properties": {
                    "gefaelle": float(drainage.get("gefaelle_promille", 0.0))
                }
            }
        else:
            # Entwässerungsschacht
...
def convert_client_c_fdk_data(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    visualization_data = {
        "scene": {
            "name": "ClientC FDK Infrastrukturszene",
            "elements": []
        },
        "meta": data.get("meta", {})
    }
    ...
    for entwaesserung in elements.get("entwässerungssysteme", []):
        ...
        if is_shaft:
            ...
            entwaesserung_element = {
                "type": "drainage_pipe",
                "id": entwaesserung.get("id", "unknown"),
                "name": entwaesserung.get("name", f"Entwässerung {entwaesserung.get('id', 'unknown')}"),
                "geometry": {
                    "type": "pipe",
                    "start": position,
                    "end": end_position,
