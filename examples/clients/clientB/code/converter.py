...
from pyarm.core.enums.process_enums import ProcessEnum, ElementType, UnitEnum
from pyarm.core.models.base_models import Parameter, InfrastructureElement
    ...
    def _convert_foundation(
        ...
        for item in data:
            ...
            parameters = [
                ...
                Parameter(
                    name="Typ",
                    value=item.get("fundament_typ"),
                    process=ProcessEnum.FOUNDATION_TYPE,
                ),
                Parameter(
                    name="X",
                    value=float(item.get("x_wert", 0)),
                    process=ProcessEnum.X_COORDINATE,
                    unit=UnitEnum.METER,
                ),
                Parameter(
                    name="Y",
                    value=float(item.get("y_wert", 0)),
                    process=ProcessEnum.Y_COORDINATE,
                    unit=UnitEnum.METER,
                ),
                Parameter(
                    name="Z",
                    value=float(item.get("z_wert", 0)),
                ...
                Parameter(
                    name="Tiefe",
                    value=float(item.get("tiefe_m", 0)),
                    process=ProcessEnum.FOUNDATION_DEPTH,
                    unit=UnitEnum.METER,
                ),
                Parameter(
                    name="Höhe",
                    value=float(item.get("hoehe_m", 0)),
    ...
    def _convert_mast(
        ...
        for item in data:
            ...
            parameters = [
                ...
                Parameter(
                    name="Azimut",
                    value=float(item.get("azimut_grad", 0)),
        ...
        for item in data:
            ...
            if element_type == ElementType.DRAINAGE_PIPE.value:
                ...
                pipe_parameters = [
                    Parameter(
                        name="X2",
                        value=float(item.get("x_wert_ende", 0)),
                        process=ProcessEnum.X_COORDINATE_END,
                        unit=UnitEnum.METER,
                    ),
                    Parameter(
                        name="Y2",
                        value=float(item.get("y_wert_ende", 0)),
                        process=ProcessEnum.Y_COORDINATE_END,
                        unit=UnitEnum.METER,
                    ),
                    Parameter(
                        name="Z2",
                        value=float(item.get("z_wert_ende", 0)),
                    ...
                    Parameter(
                        name="Durchmesser",
                        value=diameter_m,
                        process=ProcessEnum.PIPE_DIAMETER,
                        unit=UnitEnum.METER,
                    ),
                    Parameter(
                        name="Gefälle",
                        value=float(item.get("gefaelle_promille", 0)),
