"""
Client A specific data converter.
"""
...
class ClientAConverter:
    """
    Converter for Client A data.
    Converts client-specific data to the canonical format.
    """
    ...
    def can_convert(self, data: dict[str, Any]) -> bool:
        """
        Checks if this converter can convert the provided data.
    ...
    def convert(self, data: dict[str, Any]) -> list[InfrastructureElement]:
        """
        Converts the provided data into InfrastructureElement objects.
        ...
        if converter_method:
            if project_id == "project1":
                ...
                # For Project 2 we use the Project2 converters
                converter_method = getattr(self, f"_convert_{element_type}_project2", None)
                if converter_method:
                    return converter_method(raw_data, project_id)
                # Fallback to the standard converter if no specific one exists
                return self._convert_generic(raw_data, element_type)

        # Fallback to generic converter
        return self._convert_generic(raw_data, element_type)

    def _convert_foundation(self, data: list[dict[str, Any]], project_id: str = "unknown") -> list[InfrastructureElement]:
        """
        Converts foundation data from Client A (Project 1).
        ...
        -------
        list[InfrastructureElement]
            List of converted foundation elements
        """
        results = []
        for item in data:
            # Basic attributes
            ...
            results.append(factory.create_element(element_data))
    ...
    def _convert_mast_project2(self, data: list[dict[str, Any]]) -> list[InfrastructureElement]:
        """
        ...
        for item in data:
            ...
            parameters = [
                ...
                Parameter(
                    name="HeightMast",
                    value=float(item.get("HeightMast", 0)),
                    process=ProcessEnum.HEIGHT,
                    unit=UnitEnum.METER,
                ),
                Parameter(
    ...
    def _convert_track(self, data: list[dict[str, Any]]) -> list[InfrastructureElement]:
        ...
        for item in data:
            ...
            parameters = [
                ...
                Parameter(
                    name="Spurweite",
                    value=float(item.get("Spurweite", 0)),
    ...
    def _convert_track_project2(self, data: list[dict[str, Any]]) -> list[InfrastructureElement]:
        """
        ...
        for item in data:
            ...
            parameters = [
                ...
                Parameter(
                    name="East2",
                    value=float(item.get("East2", 0)),
                    process=ProcessEnum.X_COORDINATE_END,
                    unit=UnitEnum.METER,
                ),
                Parameter(
                    name="North2",
                    value=float(item.get("North2", 0)),
                    process=ProcessEnum.Y_COORDINATE_END,
                    unit=UnitEnum.METER,
                ),
                Parameter(
                    name="Height2",
                    value=float(item.get("Height2", 0)),
                    process=ProcessEnum.Z_COORDINATE_END,
                    unit=UnitEnum.METER,
                ),
                Parameter(
                    name="TrackGauge",
                    value=float(item.get("TrackGauge", 0)),
                    process=ProcessEnum.TRACK_GAUGE,
                    unit=UnitEnum.METER,
                ),
                Parameter(
                    name="TrackType",
                    value=item.get("TrackType"),
                    process=ProcessEnum.TRACK_TYPE,
                ),
                Parameter(
                    name="TrackCant",
                    value=float(item.get("TrackCant", 0)),
                    process=ProcessEnum.TRACK_CANT,
                    unit=UnitEnum.MILLIMETER,
                ),
            ]
        ...
        for item in data:
            ...
            element_data = {
                "name": item.get(
                    ...
                    item.get(
                        "Bezeichnung",
                        f"Element {item.get('UUID', item.get('ID', 'Unknown'))}",
                    ),
                ),
