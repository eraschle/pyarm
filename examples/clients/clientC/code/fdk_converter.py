...
from pyarm.models.parameter import Parameter, UnitEnum
        ...
        for item in data:
            ...
            parameters = [
                ...
                Parameter(
                    ...
                    process=ProcessEnum.VOLUME,
                    unit=UnitEnum.CUBIC_METER,
                ),
                Parameter(
                    name="capacity_kn",
                    value=item.get("capacity_kn"),
                    process=ProcessEnum.CAPACITY,
                ...
                parameters.extend(
                    [
                        Parameter(
                            name="km",
                            value=item.get("km"),
                            process=ProcessEnum.KILOMETER_POSITION,
                            unit=UnitEnum.KILOMETER,
                        ),
                        Parameter(
                            name="gleis_id",
                            value=item.get("gleis_id"),
                            process=ProcessEnum.TRACK_REFERENCE,
                        ),
                        Parameter(
                            name="distance_m",
                            value=item.get("distance_m"),
                            process=ProcessEnum.DISTANCE,
                            unit=UnitEnum.METER,
                        ),
                    ]
                )
            ...
            if "ifc_data" in item:
                ...
                parameters.extend(
                    [
                        Parameter(
                            name="ifc_global_id",
                            value=ifc.get("globalId"),
                            process=ProcessEnum.IFC_GLOBAL_ID,
                        ),
                        Parameter(
                            name="ifc_type",
                            value=ifc.get("type"),
                            process=ProcessEnum.IFC_TYPE,
                        ),
                        Parameter(
                            name="ifc_material",
                            value=ifc.get("material"),
                            process=ProcessEnum.IFC_MATERIAL,
                        ),
                        Parameter(
                            name="ifc_category",
                            value=ifc.get("category"),
                            process=ProcessEnum.IFC_CATEGORY,
                        ),
                    ]
                )
                ...
                parameters.extend(
                    [
                        Parameter(
                            name="bauphase_id",
                            value=bp.get("id"),
                            process=ProcessEnum.CONSTRUCTION_PHASE_ID,
                        ),
                        Parameter(
                            name="bauphase_name",
                            value=bp.get("name"),
                            process=ProcessEnum.CONSTRUCTION_PHASE_NAME,
                        ),
                        Parameter(
                            name="bauphase_start",
                            value=bp.get("start_date"),
                            process=ProcessEnum.CONSTRUCTION_PHASE_START,
                        ),
                        Parameter(
                            name="bauphase_end",
                            value=bp.get("end_date"),
                            process=ProcessEnum.CONSTRUCTION_PHASE_END,
                        ),
                    ]
                )
    ...
    def _convert_track(self, data: List[Dict[str, Any]]) -> List[InfrastructureElement]:
        ...
        for item in data:
            ...
            parameters = [
                ...
                Parameter(
                    name="length_m",
                    value=item.get("length_m"),
                    process=ProcessEnum.LENGTH,
                    unit=UnitEnum.METER,
                ),
                Parameter(
                    name="gauge_mm",
                    value=item.get("gauge_mm"),
        ...
        for item in data:
            ...
            parameters = [
                ...
                Parameter(
                    name="type",
                    value=item.get("type"),
                    process=ProcessEnum.DRAINAGE_TYPE,
                ),
            ]
            ...
            if element_type == ElementType.DRAINAGE_SHAFT.value:
                ...
                parameters.extend(
                    [
                        ...
                        Parameter(
                            name="km_start",
                            value=item.get("km_start"),
                            process=ProcessEnum.START_KILOMETER,
                            unit=UnitEnum.KILOMETER,
                        ),
                        Parameter(
                            name="km_end",
                            value=item.get("km_end"),
                            process=ProcessEnum.END_KILOMETER,
                            unit=UnitEnum.KILOMETER,
                        ),
                    ]
                )
    ...
    def _convert_building(self, data: List[Dict[str, Any]]) -> List[InfrastructureElement]:
        ...
        for item in data:
            ...
            element_data = {
                ...
                "uuid": item.get("id"),
                "element_type": ElementType.BUILDING.value,
                "parameters": [],
            }
            ...
            parameters = [
                ...
                Parameter(
                    ...
                    process=ProcessEnum.DESCRIPTION,
                ),
                Parameter(
                    name="year_built",
                    value=item.get("year_built"),
                    process=ProcessEnum.YEAR_BUILT,
                ),
                Parameter(
                    name="renovation_year",
                    value=item.get("renovation_year"),
                    process=ProcessEnum.RENOVATION_YEAR,
                ),
            ...
            if "coord_x" in item:
                parameters.extend(
                    [
                        Parameter(
                            name="coord_x",
                            value=item.get("coord_x"),
                            process=ProcessEnum.X_COORDINATE,
                            unit=UnitEnum.METER,
                        ),
                        Parameter(
                            name="coord_y",
                            value=item.get("coord_y"),
                            process=ProcessEnum.Y_COORDINATE,
                            unit=UnitEnum.METER,
                        ),
                        Parameter(
                            name="coord_z",
                            value=item.get("coord_z"),
            ...
            for key, value in item.items():
                ...
                if key == "ifc_data" and isinstance(value, dict):
                    for ifc_key, ifc_value in value.items():
                        ...
                        for pe in ProcessEnum:
                            if (
                                pe.name.lower() == ifc_param_name.lower()
                                or pe.name.lower() in ifc_param_name.lower()
                            ):
                                process_enum = pe
                                break
                        parameters.append(
                ...
                if key == "bauphase" and isinstance(value, dict):
                    for bp_key, bp_value in value.items():
                        ...
                        for pe in ProcessEnum:
                            if (
                                pe.name.lower() == bp_param_name.lower()
                                or pe.name.lower() in bp_param_name.lower()
                            ):
                                process_enum = pe
                                break
                        parameters.append(
                            Parameter(name=bp_param_name, value=bp_value, process=process_enum)
                        )
                    continue
