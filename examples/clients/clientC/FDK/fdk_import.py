...
class FdkImportProcess:
    ...
    def run(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not fdk_data:
                ...
                return {
                    "status": "error",
                    "message": "Keine gültigen FDK-Daten gefunden.",
                }

            logger.info(
                "FDK-Daten erfolgreich gelesen. "
                f"Gefundene Elementtypen: {self.reader.get_element_types(fdk_data)}"
            )
        except Exception as e:
            logger.error(f"Fehler beim Lesen der FDK-Daten: {str(e)}")
            return {
                "status": "error",
                "message": f"Fehler beim Lesen der FDK-Daten: {str(e)}",
            }
        ...
        if "meta" in fdk_data and fdk_data["meta"]:
            ...
            logger.info(
                f"Metadaten gefunden: Version {meta_info.get('version', 'unbekannt')}, "
                f"Projekt: {meta_info.get('projektName', 'unbekannt')}"
            )
        ...
        conversion_stats = {"total_elements": 0, "converted_elements": 0, "by_type": {}}
        ...
        for element_type in self.reader.get_element_types(fdk_data):
            ...
            try:
                if self.converter.can_convert(type_data):
                    ...
                    logger.info(
                        f"{len(converted_elements)} Elemente vom Typ {element_type} konvertiert"
                    )
                    conversion_stats["by_type"][element_type]["converted_count"] = len(
                        converted_elements
                    )
        ...
        if meta_info or bauphasen_info:
            ...
            with open(metadata_output, "w", encoding="utf-8") as f:
                json.dump(
                    {"meta": meta_info, "bauphasen": bauphasen_info},
                    f,
                    indent=2,
                    ensure_ascii=False,
                )
    ...
    def _generate_visualization_data(self, repository: JsonElementRepository) -> Dict[str, Any]:
        ...
        visualization_data = {
            "summary": {"total_elements": len(elements), "type_counts": type_counts},
            "elements_by_type": {},
        }
        ...
        for element_type, elements_list in elements_by_type.items():
            ...
            for element in elements_list:
                ...
                ifc_global_id = element.get_param("IFC_GLOBAL_ID") or element.get_param(
                    "ifc_global_id"
                )
                if ifc_global_id:
                    element_info["ifc"] = {
                        ...
                        "category": element.get_param("IFC_CATEGORY")
                        or element.get_param("ifc_category"),
                    }

                # Bauphasendaten extrahieren, falls vorhanden
                bauphase_id = element.get_param("CONSTRUCTION_PHASE_ID") or element.get_param(
                    "bauphase_id"
                )
                if bauphase_id:
                    element_info["construction_phase"] = {
                        "id": bauphase_id,
                        "name": element.get_param("CONSTRUCTION_PHASE_NAME")
                        or element.get_param("bauphase_name"),
                        "start": element.get_param("CONSTRUCTION_PHASE_START")
                        or element.get_param("bauphase_start"),
                        "end": element.get_param("CONSTRUCTION_PHASE_END")
                        or element.get_param("bauphase_end"),
                    }
