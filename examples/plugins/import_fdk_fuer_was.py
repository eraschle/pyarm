#!/usr/bin/env python3
"""
Eigenstaendiges Skript fuer den FDK-Import-Prozess.
Importiert und verarbeitet FDK-Daten ohne Abhaengigkeiten von anderen Clientmodulen.
"""
...
# Fuege das aktuelle Verzeichnis zum Python-Pfad hinzu
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Verzeichnis fuer das Repository",
    )
...
def run_fdk_import_process(fdk_file, repository_dir, output_dir):
    ...
    try:
        ...
        from pyarm.processes.process3_fdk_import import FdkImportProcess

        logger.info(f"Starte FDK-Import-Prozess fuer Datei: {fdk_file}")
        ...
        if result["status"] == "success":
            ...
            if "conversion_stats" in result and "by_type" in result["conversion_stats"]:
                for element_type, stats in result["conversion_stats"]["by_type"].items():
                    logger.info(
                        f"  - {element_type}: {stats['converted_count']} von {stats['raw_count']} Elementen konvertiert"
                    )

            if "metadata" in result:
                meta = result["metadata"]
                logger.info(
                    f"Projektinformation: {meta.get('project', 'Unbekannt')}, Version: {meta.get('version', 'Unbekannt')}"
                )
        ...
        logger.error(f"Fehler beim Importieren oder Ausfuehren des FDK-Prozesses: {str(e)}")
        import traceback
