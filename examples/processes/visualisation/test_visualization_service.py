...
class TestVisualizationService(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mast.set_point(east=100.0, north=200.0, altitude=305.0)
        self.cantilever.set_param(ProcessEnum.CANTILEVER_TYPE, "Standard")
        self.cantilever.set_param(ProcessEnum.LENGTH, 3.5)
        self.cantilever.set_param(ProcessEnum.MATERIAL, "Aluminium")

        # Gleis
        self.track = Track(name="TestTrack")
        self.mast.set_point(east=100.0, north=200.0, altitude=300.0)
        self.cantilever.set_param(ProcessEnum.X_COORDINATE_END, 200.0)
        self.cantilever.set_param(ProcessEnum.Y_COORDINATE_END, 300.0)
        self.cantilever.set_param(ProcessEnum.Z_COORDINATE_END, 310.0)
        self.cantilever.set_param(ProcessEnum.TRACK_TYPE, "UIC60")
        self.cantilever.set_param(ProcessEnum.TRACK_GAUGE, 1.435)
        self.cantilever.set_param(ProcessEnum.TRACK_CANT, 100.0)  # 100mm Überhöhung
        ...
        self.mast.set_point(east=200.0, north=300.0, altitude=305.0)
        self.cantilever.set_param(ProcessEnum.X_COORDINATE_END, 300.0)
        self.cantilever.set_param(ProcessEnum.Y_COORDINATE_END, 400.0)
        self.cantilever.set_param(ProcessEnum.Z_COORDINATE_END, 310.0)
        self.cantilever.set_param(ProcessEnum.TRACK_TYPE, "UIC60")
        self.cantilever.set_param(ProcessEnum.TRACK_CANT, 120.0)  # 120mm Überhöhung
        self.cantilever.set_param(ProcessEnum.TRACK_GAUGE, 1.435)
        self.cantilever.set_param(ProcessEnum.CLOTHOID_PARAMETER, 300.0)
        self.cantilever.set_param(ProcessEnum.START_RADIUS, 0.0)  # Startradius der Klothoide
        self.cantilever.set_param(ProcessEnum.END_RADIUS, 800.0)
        self.curved_track.set_param(ProcessEnum.MATERIAL, "Stahl")

        # Joch zwischen zwei Masten
        self.mast2 = Mast(name="TestMast2")
        self.mast.set_point(east=120.0, north=200.0, altitude=300.0)
        self.cantilever.set_param(ProcessEnum.HEIGHT, 15.0)
        self.cantilever.set_param(ProcessEnum.MAST_TYPE, "DP20")
        self.mast2.set_param(ProcessEnum.MATERIAL, "Stahl")

        self.joch = Joch(name="TestJoch")
        self.mast.set_point(
            east=100.0, north=200.0, altitude=315.0
        )  # 15m über dem Boden (Masthöhe)
        self.cantilever.set_param(ProcessEnum.X_COORDINATE_END, 120.0)
        self.cantilever.set_param(ProcessEnum.Y_COORDINATE_END, 200.0)
        self.cantilever.set_param(ProcessEnum.Z_COORDINATE_END, 315.0)
        self.cantilever.set_param(ProcessEnum.JOCH_TYPE, "Standard")
        self.cantilever.set_param(ProcessEnum.JOCH_SPAN, 20.0)
    ...
    def test_element_position(self):
        ...
        element_with_azimuth.set_point(east=100.0, north=200.0, altitude=300.0)
    ...
    def test_mast_visualization(self):
        ...
        mast_with_foundation.set_point(east=100.0, north=200.0, altitude=300.0)
    ...
    def test_calculate_clothoid_points(self):
        ...
        self.assertAlmostEqual(first_point[0], track.position.x or 0.0)
        self.assertAlmostEqual(first_point[1], track.position.y or 0.0)
        self.assertAlmostEqual(first_point[2], track.position.z or 0.0)
        assert track is not None and track.position.z is not None
        assert track.get_param(ProcessEnum.X_COORDINATE_END, None)
        assert track.get_param(ProcessEnum.Y_COORDINATE_END, None)
        x_end = track.get_param(ProcessEnum.X_COORDINATE_END, None)
        y_end = track.get_param(ProcessEnum.Y_COORDINATE_END, None)
        y_end = track.get_param(ProcessEnum.Z_COORDINATE_END, None)
        ...
        track_length = (x_end - track.position.x) ** 2 + (y_end - track.position.nory * 2) ** 0.5
        ...
        if abs(end_station - track_length) < 0.001:
            self.assertAlmostEqual(last_point[0], x_end, places=1)
            self.assertAlmostEqual(last_point[1], y_end, places=1)
            self.assertAlmostEqual(last_point[2], y_end, places=1)
