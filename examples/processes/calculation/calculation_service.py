...
from pyarm.core.models.components import ComponentType, ElementReference
from pyarm.core.models.element_models import LinearElement
from pyarm.core.models.location import Dimension
...
class CalculationService:
    ...
    def _prepare_element_for_calculation(
        ...
        material = element.get_components_by_type(ComponentType.MATERIAL)
        if material:
        ...
        if position:
            calc_element.add_property(
                "position",
                {"x": position.x, "y": position.y, "z": position.z, "azimuth": position.azimuth},
            )
        else:
        ...
        line_element = element.location
        if line_element:
            calc_element.add_property(
                "end_position",
                {
                    "x": line_element.end.x,
                    "y": line_element.end.y,
                    "z": line_element.end.z,
                },
            )
        ...
        if dimension and isinstance(dimension, Dimension):
            dim_dict = {}
    ...
    def _add_mast_calculation(
        ...
        if dimension and isinstance(dimension, Dimension):
            ...
            height = element.get_param(ProcessEnum.MAST_HEIGHT, 0.0) or 0.0
    ...
    def _add_linear_element_calculation(
        ...
        if start_pos and end_pos:
            ...
            # Richtung berechnen
            if length > 0:
                direction = {"dx": (x2 - x1) / length, "dy": (y2 - y1) / length}
    ...
    def _add_curved_track_calculation(
        ...
        # else:
        #     radius = element.get_param(ProcessEnum.END_RADIUS, 0.0)
        start_radius = None
    ...
    def _add_pipe_calculation(
        ...
        component = element.get_component("dimension")
        if component and isinstance(component, Dimension):
    ...
    def _add_shaft_calculation(
        ...
        # Eigenschaften
        calc_element.add_property(
            "shaft_material", element.get_param(ProcessEnum.SHAFT_MATERIAL, "")
        )
