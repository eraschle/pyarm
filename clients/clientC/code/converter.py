    ...
    def _convert_foundation(self, data: List[Dict[str, Any]]) -> List[InfrastructureElement]:
        ...
        for item in data:
            ...
            parameters = [
                ...
                Parameter(name="coord_x", value=float(item.get("coord_x", 0)), process=ProcessEnum.X_COORDINATE, unit=UnitEnum.METER),
                Parameter(name="coord_y", value=float(item.get("coord_y", 0)), process=ProcessEnum.Y_COORDINATE, unit=UnitEnum.METER),
                Parameter(name="coord_z", value=float(item.get("coord_z", 0)), process=ProcessEnum.Z_COORDINATE, unit=UnitEnum.METER),
                # Umrechnung von Millimetern in Meter
                Parameter(name="width_mm", value=float(item.get("width_mm", 0))/1000, process=ProcessEnum.FOUNDATION_WIDTH, unit=UnitEnum.METER),
                Parameter(name="depth_mm", value=float(item.get("depth_mm", 0))/1000, process=ProcessEnum.FOUNDATION_DEPTH, unit=UnitEnum.METER),
                Parameter(name="height_mm", value=float(item.get("height_mm", 0))/1000, process=ProcessEnum.FOUNDATION_HEIGHT, unit=UnitEnum.METER),
    ...
    def _convert_mast(self, data: List[Dict[str, Any]]) -> List[InfrastructureElement]:
        ...
        for item in data:
            ...
            parameters = [
                ...
                Parameter(name="coord_z", value=float(item.get("coord_z", 0)), process=ProcessEnum.Z_COORDINATE, unit=UnitEnum.METER),
                # Umrechnung von Millimetern in Meter
                Parameter(name="height_mm", value=float(item.get("height_mm", 0))/1000, process=ProcessEnum.MAST_HEIGHT, unit=UnitEnum.METER),
                Parameter(name="azimuth", value=float(item.get("azimuth", 0)), process=ProcessEnum.AZIMUTH, unit=UnitEnum.DEGREE),
                Parameter(name="material", value=item.get("material"), process=ProcessEnum.MATERIAL),
                Parameter(name="profile", value=item.get("profile"), process=ProcessEnum.MAST_PROFILE_TYPE),
            ]
    ...
    def _convert_joch(self, data: List[Dict[str, Any]]) -> List[InfrastructureElement]:
        ...
        for item in data:
            ...
            parameters = [
                ...
                Parameter(name="coord_x1", value=float(item.get("coord_x1", 0)), process=ProcessEnum.X_COORDINATE, unit=UnitEnum.METER),
                Parameter(name="coord_y1", value=float(item.get("coord_y1", 0)), process=ProcessEnum.Y_COORDINATE, unit=UnitEnum.METER),
                Parameter(name="coord_z1", value=float(item.get("coord_z1", 0)), process=ProcessEnum.Z_COORDINATE, unit=UnitEnum.METER),
                Parameter(name="coord_x2", value=float(item.get("coord_x2", 0)), process=ProcessEnum.X_COORDINATE_END, unit=UnitEnum.METER),
                Parameter(name="coord_y2", value=float(item.get("coord_y2", 0)), process=ProcessEnum.Y_COORDINATE_END, unit=UnitEnum.METER),
                Parameter(name="coord_z2", value=float(item.get("coord_z2", 0)), process=ProcessEnum.Z_COORDINATE_END, unit=UnitEnum.METER),
                # Umrechnung von Millimetern in Meter
                Parameter(name="span_mm", value=float(item.get("span_mm", 0))/1000, process=ProcessEnum.JOCH_SPAN, unit=UnitEnum.METER),
                Parameter(name="material", value=item.get("material"), process=ProcessEnum.MATERIAL),
            ]
    ...
    def _convert_track(self, data: List[Dict[str, Any]]) -> List[InfrastructureElement]:
        ...
        for item in data:
            ...
            parameters = [
                ...
                Parameter(name="coord_z2", value=float(item.get("coord_z2", 0)), process=ProcessEnum.Z_COORDINATE_END, unit=UnitEnum.METER),
                # Umrechnung von Millimetern in Meter
                Parameter(name="gauge_mm", value=float(item.get("gauge_mm", 0))/1000, process=ProcessEnum.TRACK_GAUGE, unit=UnitEnum.METER),
                Parameter(name="type", value=item.get("type"), process=ProcessEnum.TRACK_TYPE),
                Parameter(name="cant_mm", value=float(item.get("cant_mm", 0)), process=ProcessEnum.TRACK_CANT, unit=UnitEnum.MILLIMETER),
            ]
    ...
    def _convert_curved_track(self, data: List[Dict[str, Any]]) -> List[InfrastructureElement]:
        ...
        for item in data:
            ...
            parameters = [
                ...
                Parameter(name="type", value=item.get("type"), process=ProcessEnum.TRACK_TYPE),
                Parameter(name="cant_mm", value=float(item.get("cant_mm", 0)), process=ProcessEnum.TRACK_CANT, unit=UnitEnum.MILLIMETER),
                Parameter(name="clothoid_param", value=float(item.get("clothoid_param", 0)), process=ProcessEnum.CLOTHOID_PARAMETER),
                Parameter(name="start_radius_m", value=start_radius, process=ProcessEnum.START_RADIUS, unit=UnitEnum.METER),
                Parameter(name="end_radius_m", value=float(item.get("end_radius_m", 0)), process=ProcessEnum.END_RADIUS, unit=UnitEnum.METER),
            ]
    ...
    def _convert_drainage(self, data: List[Dict[str, Any]]) -> List[InfrastructureElement]:
        ...
        for item in data:
            ...
            common_parameters = [
                ...
                Parameter(name="diameter_mm", value=float(item.get("diameter_mm", 0)), process=ProcessEnum.DIAMETER, unit=UnitEnum.MILLIMETER),
            ]
            ...
            if element_type == ElementType.DRAINAGE_PIPE.value:
                ...
                pipe_parameters = [
                    ...
                    Parameter(name="coord_y2", value=float(item.get("coord_y2", 0)), process=ProcessEnum.Y_COORDINATE_END, unit=UnitEnum.METER),
                    Parameter(name="coord_z2", value=float(item.get("coord_z2", 0)), process=ProcessEnum.Z_COORDINATE_END, unit=UnitEnum.METER),
                    Parameter(name="slope_permille", value=float(item.get("slope_permille", 0)), process=ProcessEnum.PIPE_SLOPE, unit=UnitEnum.PROMILLE),
                ]
                parameters = common_parameters + pipe_parameters
            else:  # DRAINAGE_SHAFT
                # Spezifische Parameter für Schächte
                shaft_parameters = [
                    Parameter(name="diameter_mm", value=float(item.get("diameter_mm", 0)), process=ProcessEnum.SHAFT_DIAMETER, unit=UnitEnum.MILLIMETER),
                ]
