...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=str, 
        required=True,
        help="Verzeichnis mit ClientA-Dateien"
    )
    parser.add_argument(
        "--project",
        type=str,
...
def process_client_a_files(input_dir: str, project: str) -> Dict[str, Any]:
    ...
    try:
        ...
        for proj in projects_to_process:
            ...
            # Statistiken für das Projekt hinzufügen
            processed_data["projects"][proj]["statistics"] = {
                "total_elements": sum(len(elements) for elements in processed_data["projects"][proj]["elements"].values()),
                "element_counts": {element_type: len(elements) for element_type, elements in processed_data["projects"][proj]["elements"].items()}
            }
        
        # Gesamtstatistiken hinzufügen
        total_elements = sum(proj_data["statistics"]["total_elements"] for proj_data in processed_data["projects"].values())
...
def run_client_a_import(input_dir: str, project: str, output_dir: str) -> bool:
    """
    Führt den ClientA-Import-Prozess aus.
    ...
    try:
        ...
        logger.info(f"Verarbeite ClientA-Dateien aus {input_dir} für Projekt {project}...")
        processed_data = process_client_a_files(input_dir, project)
        
        if not processed_data or not processed_data.get("projects"):
            logger.error("Keine gültigen ClientA-Daten gefunden.")
            return False
        
        # Statistiken anzeigen
        stats = processed_data["statistics"]
        logger.info(f"Insgesamt {stats['total_elements']} Elemente in {stats['projects_processed']} Projekten verarbeitet")
...
def main():
    ...
    # ClientA-Import-Prozess ausführen
