...
from ..common.models.element_models import Joch, Mast
...
class VisualizationService:
    ...
    def calculate_clothoid_points(
        self, element_id: Union[UUID, str], start_station: float, end_station: float, step: float
        ...
        start_radius = None
        end_radius = None
        if element or not has_clothoid_capability(element):
            # Klothoidenparameter extrahieren
            clothoid = element.get_param(ProcessEnum.CLOTHOID_PARAMETER, 0.0)
            start_radius = element.get_param(ProcessEnum.START_RADIUS, float("inf"))
            end_radius = element.get_param(ProcessEnum.END_RADIUS, 0.0)
